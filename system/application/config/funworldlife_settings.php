<?php  if (!defined('BASEPATH')) exit('No direct script access allowed');

	$config["FWL_starting_health"] = 100;
	$config["FWL_starting_money"] = 50;

	//$config["FWL_images"] = base_url() . "/images/";
	
	$config["FWL_clean_street_chance"] = 5;
	$config["FWL_clean_street_money"] = 3;
	
	$config["FWL_house_image_width"] = 186;
	$config["FWL_house_image_height"] = 284;
	$config["FWL_house_price_color_regular"] = 328;
	$config["FWL_house_price_color_gold"] = 476;
	
	$config["FWL_products_field_limit"] = 1000;
	
	$config["FWL_seconds_between_digs"] = 25;
	$config["FWL_dig_chance_old_shoe"] = 20;
	$config["FWL_dig_chance_new_shoe"] = 40;
	$config["FWL_dig_chance_gold"] = 1000;
	$config["FWL_dig_chance_1000_funcoins"] = 260;
	$config["FWL_dig_product_id_old_shoe"] = 18;
	$config["FWL_dig_product_id_new_shoe"] = 19;
	$config["FWL_dig_product_id_gold"] = 20;
?>